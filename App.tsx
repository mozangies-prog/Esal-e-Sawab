...
const POLLING_INTERVAL = 5000; // Poll every 5 seconds for live updates
...
const App: React.FC = () => {
  const [data, setData] = useState<EsalData>(() => {
    try {
      ...
      logger.error("Local storage load failed", e);
    }
  ...
  const [dbStatus, setDbStatus] = useState<'live' | 'offline' | 'syncing'>('syncing');
  const [aiInsight, setAiInsight] = useState<string>("Bismillah. Every recitation is a gift for your loved ones.");
  const [insightLoading, setInsightLoading] = useState(false);

  // Sync Logic: Fetching live data from MySQL via Polling
  const fetchLiveData = async () => {
    setDbStatus('syncing');
    const [stats, contribs] = await Promise.all([
      apiService.getStats(),
    ...
    if (contribs && Array.isArray(contribs)) {
      setData(prev => ({ ...prev, contributions: contribs }));
    }
  };

  useEffect(() => {
    fetchLiveData(); // Initial fetch
    const timer = setInterval(fetchLiveData, POLLING_INTERVAL);
  ...
  const handleAdd = async (type: RecitationType, count: number) => {
    ...
    setData(prev => ({ ...prev, contributions: [newContrib, ...prev.contributions] }));

    // 2. Persistent Update (Remote MySQL)
    const success = await apiService.postContribution(newContrib);
    
    if (success) {
      fetchLiveData(); // Immediate refresh after posting
    } else {
      logger.warn("Server sync failed. Data is local only.");
    }
  ...
  const totals = useMemo(() => {
    ...
    Object.values(RecitationType).forEach(t => {
      ...
      map[t] = globalStats ? (globalStats[dbKey] || 0) : 0;
    });
    return map;
  }, [globalStats]);

  const grandTotal = globalStats?.grandTotal || 0;

  const groupedContributions = useMemo(() => {
    const groups: Record<string, Contribution[]> = {};
    data.contributions.forEach(c => {
      const date = new Date(c.timestamp).toLocaleDateString(undefined, { day: 'numeric', month: 'long', year: 'numeric' });
  ...
  return (
    <div className="min-h-screen pb-12 px-4 sm:px-6 lg:px-8 pt-6 max-w-[1600px] mx-auto">
      {/* Insight Banner */}
      <div className="mb-6 bg-cyan-50/50 border border-cyan-100 rounded-2xl p-4 animate-fade-in relative overflow-hidden group">
        ...
        <div className="flex items-start gap-3 relative z-10">
          ...
          <div>
            ...
            <p className="text-sm font-medium text-slate-700 italic leading-relaxed">"{aiInsight}"</p>
          </div>
        </div>
      </div>
      ...
      <div className="flex flex-col lg:flex-row justify-between items-center gap-6 mb-8">
        <div className="text-center lg:text-left">
          ...
          <p className="text-slate-400 text-xs font-medium uppercase tracking-[0.3em]">Runtime Live Stats (MySQL)</p>
        </div>
        ...
        <div className="flex flex-wrap justify-center gap-4">
          <div className="bg-white rounded-2xl shadow-sm border border-cyan-50 p-3 px-6 flex items-center gap-4">
            ...
            <span className="text-cyan-500 font-black text-2xl leading-none">{grandTotal.toLocaleString()}</span>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-cyan-50 p-3 px-6 flex items-center gap-4 min-w-[280px]">
        ...
        <div className="flex items-center gap-4 w-full max-w-md">
          ...
          <div className="flex-1">
            ...
            <input
              ...
              className="w-full bg-transparent border-b border-slate-100 py-1 outline-none text-slate-700 font-bold text-sm focus:border-cyan-400 transition-all"
            />
          </div>
        </div>

        <div className="flex bg-slate-100/50 rounded-xl p-1.5 border border-slate-200/50">
          <button onClick={() => setViewMode('grid')} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'grid' ? 'bg-white text-cyan-500 shadow-md' : 'text-slate-400'}`}>
            <i className="fas fa-shapes"></i> Grid
          </button>
          <button onClick={() => setViewMode('list')} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${viewMode === 'list' ? 'bg-white text-cyan-500 shadow-md' : 'text-slate-400'}`}>
      ...
      <div className="bg-white rounded-3xl border border-cyan-50 p-8 shadow-sm max-w-5xl mx-auto overflow-hidden relative">
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-slate-400 text-[10px] font-bold uppercase tracking-[0.2em] mt-1">Live updates from Railway MySQL</p>
          </div>
          <div className="h-px bg-slate-100 flex-grow mx-8 hidden sm:block"></div>
          <div className="text-right">
            <span className="text-[10px] font-bold text-slate-300 uppercase">Server Status</span>
            <div className="flex items-center gap-2 justify-end">
              <div className={`w-2 h-2 rounded-full ${dbStatus === 'live' ? 'bg-green-400 animate-pulse' : dbStatus === 'syncing' ? 'bg-yellow-400 animate-bounce' : 'bg-red-400'}`}></div>
        ...
        <div className="max-h-[400px] overflow-y-auto pr-4 custom-scrollbar">
          {data.contributions.length === 0 ? (
            ...
            <div className="space-y-6">
              {Object.entries(groupedContributions).map(([date, contributions]) => (
                <div key={date}>
                  <p className="text-[10px] font-black text-cyan-400 uppercase tracking-[0.3em] mb-4 border-b pb-1 inline-block">{date}</p>
                  <div className="space-y-3">
                    {contributions.map((c) => (
                      <div key={c.id} className={`flex justify-between items-center p-3 px-5 rounded-2xl bg-slate-50 transition-all ${lastAddedId === c.id ? 'bg-cyan-50 ring-2 ring-cyan-100' : ''}`}>
                        <div className="flex items-center gap-4">
                          <div className="w-10 h-10 rounded-xl bg-white text-slate-400 flex items-center justify-center border border-slate-100"><i className="fas fa-check text-xs"></i></div>
                          <div>
                            <p className="text-sm font-black text-slate-700">{c.contributorName}</p>
                            <p className="text-[10px] font-bold text-slate-400 uppercase">{c.recitationType}</p>
                          </div>
                        </div>
                        <div className="text-right">
                          <p className="text-cyan-500 font-black text-lg">+{c.count.toLocaleString()}</p>
                          <p className="text-[10px] text-slate-300 uppercase">{new Date(c.timestamp).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</p>
                        </div>
                      </div>
                    ))}
                  </div>
      ...
      <footer className="text-center py-12 mt-12 border-t border-slate-100">
        ...
        <p className="text-[8px] text-slate-200 mt-4 uppercase tracking-[0.2em]">Railway API Polling Active (5s) • v2.1.0</p>
      </footer>
    </div>
  );
};
